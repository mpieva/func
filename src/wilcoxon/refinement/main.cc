...
int main( int argc, char *argv[] ) 
{
	...
	if (  argc != 11 ) {
		cerr << "Usage: " << argv[0] << " infile_ann "
				...
				<< "       graph_path.txt term2term.txt GO_ID pval_after" << endl 
				<< "       cutoff" << endl ;
		exit( 1 ) ;
	}
	...
	go_graph graph( trans, term2term, id_to_go, cutoff ) ;
	term2term.close(  ) ;
	...
	/*****************
         *  do refinement
	 *******************/
	graph.do_refin( parent_go, pval_before, pval_after, gns.sumnties() ) ;
